...
async function main() {
  ...
  const regReceipt = await waitForReceipt(registerTx, "Registration");
  ...
  if (!transferLogs.length)
    throw new Error("No Transfer event found — registration may have failed");
  ...
  const valReqTx = await ownerWalletClient.writeContract({
  ...
  const valResTx = await validatorWalletClient.writeContract({
    address: VALIDATION_REGISTRY,
    abi: validationAbi,
